...
def handleSearchGlyphList(standardGlyph, contourIndex, file, currentWindow, mainWindow):
	...
	if checkSetData[2] == 0:
		...
		if mainWindow.mode is matrixMode:
			getMatchGroupByMatrix(standardGlyph, contourIndex, margin, width, width, file, checkSetData, mainWindow.jsonFileName1,mainWindow.jsonFileName2)
			currentWindow.groupDict = findContoursGroup(checkSetData, mainWindow)

		elif mainWindow.mode is topologyMode:
			getMatchGroupByTopology(standardGlyph, contourIndex, k, currentWindow.font,checkSetData,mainWindow.jsonFileName1,mainWindow.jsonFileName2)
			currentWindow.groupDict = findContoursGroup(checkSetData, mainWindow)
